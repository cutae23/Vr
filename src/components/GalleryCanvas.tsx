...
export default function GalleryCanvas({
  ...
  useEffect(() => {
    ...
    // 2. Set beautiful Fog and background
    let bgColor = 0x0a0a14;
    if (hall.id === 'classic') bgColor = 0x1c120a;
    else if (hall.id === 'modern') bgColor = 0x0f172a;
    else if (hall.id === 'neon') bgColor = 0x05050f;

    scene.background = new THREE.Color(bgColor);
    scene.fog = new THREE.FogExp2(bgColor, 0.022);

    // 3. Construct Floor & Ceiling dynamically based on textures
    let floorMat: THREE.Material;
    let ceilingMat: THREE.Material;

    if (hall.id === 'classic') {
      // Modern parquet oak floor texture synthesized on dynamic canvas
      const canvas = document.createElement('canvas');
      canvas.width = 512;
      canvas.height = 512;
      const ctx = canvas.getContext('2d')!;
      ctx.fillStyle = '#4a2f13'; // Dark oak
      ...
      canvas.height = 256;
      const ctx = canvas.getContext('2d')!;
      ctx.fillStyle = '#1e293b'; // cool slate concrete
    ...
    // Multiplied ambient and spotlight properties for much brighter, eye-safe viewing (User feedback: "작품들이 어두워서 잘 안보이고")
    const ambientLight = new THREE.AmbientLight(
      hall.id === 'classic' ? 0xfff0dd : 
      hall.id === 'modern' ? 0xe2e8f0 : 
      hall.id === 'neon' ? 0x2d1a4e : 
      hall.id === 'nordic' ? 0xfffaf2 : 
      0xffeed5, // retro
    ...
    // Global overhead skylight projection
    const skyLight = new THREE.DirectionalLight(
      0xffffff, 
      hall.id === 'classic' ? 0.95 : 
      hall.id === 'modern' ? 1.25 : 
      hall.id === 'neon' ? 0.85 : 
      hall.id === 'nordic' ? 1.35 : 
      1.15 // retro
  ...
  const forceRefreshArtworks = () => {
    ...
    // 1. Remove all existing painting meshes
    stateRef.current.artMeshes.forEach(meshGroup => {
    ...
    artworks.forEach(art => {
      ...
      // Now asynchronously load the actual image texture
      textureLoader.load(
        art.imageUrl,
        (loadedTexture) => {
